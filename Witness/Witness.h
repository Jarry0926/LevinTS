...
private:
    ...
    static constexpr u8_t GET_DIRECTION_FROM[4] = {2, 3, 0, 1};
...
private:
    auto initContextCenter(FILE* p_FdModel, u8_t* p_Ctx) noexcept -> void;

    auto initContextEdge(FILE* p_FdModel, u8_t* p_Ctx) noexcept -> void;

    auto initContextCorner(FILE* p_FdModel, u8_t* p_Ctx) noexcept -> void;

    // [Called after reaching a goal state]
    auto isSolved() noexcept -> bool;

    // [Called after this->isSolved() returns true]
    // Trace every context and update probabilities 
    auto updateProb() noexcept -> void;
    
    auto iheapPush(f32_t p_Cost, u32_t p_Target_i) noexcept -> void;
    
    auto iheapPop(u32_t p_Top_i) noexcept -> void;

    u32_t height, width;
    i8_t* map; // a cell corresponds to a junction at its bottom-right
    f32_t probPool[1024][4] = {};
    std::vector<node_t> nodePool = {{0, 0, 0, 0.0f, -INF_F32}};
